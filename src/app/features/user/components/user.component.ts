...
import { DialogAssignRol } from './dialog-assign-rol';
...
export class UserComponent {
  ...
  assignRole(id: number){
    const dialogRef = this.dialog.open(DialogAssignRol, {
      width: '350px',
    });

    dialogRef.afterClosed().subscribe(result => {
          if (result !== undefined) {
            this.userService.assignRol(id, result).subscribe({
              next: (res) => {
              ...
              error: (err) => this.snackBarService.showSnackBar(`Error: ${err.error.error}`, 'error-snackbar', 3000, 'end', 'top')
              });
          }
        });
  }
